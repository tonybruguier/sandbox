...
class PQC(tf.keras.layers.Layer):
    ...
    def __init__(
        ...
        model_circuits: `cirq.Circuit` or List[`cirq.Circuit`] containing `sympy.Symbols` that will be
        ...
        # Ingest model_circuits.
        if isinstance(model_circuits, cirq.Circuit):
            model_circuits = [model_circuits]
        elif not isinstance(model_circuits, List):
            raise TypeError(
                "model_circuits must be a cirq.Circuit object or a list of cirq.Circuit objects."
                " Given: {}".format(model_circuits))
        ...
        if len(self._symbols_list) == 0:
            raise ValueError("model_circuits has no sympy.Symbols. Please "
