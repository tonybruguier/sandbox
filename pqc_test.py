...
class PQCTest(tf.test.TestCase, parameterized.TestCase):
    ...
    def test_pqc_model_circuit_error(self):
        ...
        with self.assertRaisesRegex(
                TypeError,
                expected_regex="model_circuits must be a cirq.Circuit"):
            pqc.PQC('junk', cirq.Z(qubit))

        with self.assertRaisesRegex(
                TypeError,
                expected_regex="model_circuits must be a cirq.Circuit"):
            pqc.PQC(list('junk'), cirq.Z(qubit))

        with self.assertRaisesRegex(
                ValueError,
                expected_regex="model_circuits has no sympy.Symbols"):
    ...
    @parameterized.parameters(
        list(
            util.kwargs_cartesian_product(backend=[None, cirq.Simulator()],
                                          repetitions=[None, 5000])))
    def test_pqc_double_learn(self, backend, repetitions):
        """Test a simple learning scenario using analytic and sample expectation
        on many backends."""
        qubit = cirq.GridQubit(0, 0)
        circuit_1 = cirq.Circuit(cirq.X(qubit)**sympy.Symbol('bit_1'))
        circuit_2 = cirq.Circuit(cirq.Z(qubit)**sympy.Symbol('bit_2'))

        quantum_datum = tf.keras.Input(shape=(), dtype=tf.dtypes.string)
        mpqc = pqc.PQC([circuit_1, circuit_2],
                       cirq.Z(qubit),
                       backend=backend,
                       repetitions=repetitions,
                       initializer=tf.keras.initializers.Constant(value=0.5))
